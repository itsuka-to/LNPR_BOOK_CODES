...
class World:
    ...
    def one_step(self, i, elems, ax):
        ...
        for obj in self.objects:
            obj.draw(ax, elems)
            if hasattr(obj, "one_step"): obj.one_step(self.time_interval)  # 追加
...
class IdealRobot:
    ...
    def draw(self, ax, elems):
        ...
        elems += ax.plot([x, xn],[y, yn], color=self.color)  # ロボットの向きを示す線分
        c = patches.Circle(xy=(x, y), radius=self.r, fill=False, color=self.color)
        elems.append(ax.add_patch(c))  # Circleをサブプロットに登録
...
if __name__ == '__main__':
    ...
    circling = Agent(0.2, 10.0/180.0*math.pi)
